...
public class GlacierCleric : Enemy
{
    ...
    protected override void AI()
    {
        ...
        else
        {
            ...
            if (!shooting)
            {
                if (Time.time > nextShot && shotCount % 3 != 0)
                {
                ...
                {
                    StartCoroutine(createShield(correctShotFrameTime));
                    shooting = true;
                    finishShoot = Time.time + shootAnimationLength;
                    walking = false;
                    shotCount++;
    ...
    private IEnumerator createShield(float time)
    {
        ...
        for (int i = 0; i < enemies2.Length; i++)
        {
            if (Mathf.Abs(tran.position.x - enemies2[i].transform.position.x) < 5 &&
                ...
                enemies2[i].name != "GlacialAegis(Clone)" && enemies2[i].name != "BishopHeal(Clone)")
            {
        ...
        if (enemiesInRange.Count > 0)
        {
            ...
            GameObject projectile = GameObject.Instantiate<GameObject>(shieldOG);
            if (projectile != null && chosen != null)
            {
                chosen.BroadcastMessage("giveShield", projectile);
            }
        }
